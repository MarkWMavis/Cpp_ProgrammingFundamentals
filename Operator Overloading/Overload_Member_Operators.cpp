...
//Overloaded Move Operator --> Much more efficient then copy constructor
...
Mystring operator+(const Mystring& lhs, const Mystring& rhs) {
	int length = strlen(lhs.get_str()) + strlen(rhs.get_str()) + 1;
	char* buff = new char[length];
	strcpy_s(buff, strlen(lhs.get_str()), lhs.get_str());
	strcat_s(buff, length * sizeof(char), rhs.get_str());
