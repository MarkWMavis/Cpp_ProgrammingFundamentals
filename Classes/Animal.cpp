...
void AnimalClassDemo() {
	// Aniaml Class Demonstration--------------------------------------------
	Animal a1("Bovine", "Cow", "Moo");
	Animal a2("Bird", "Raven", "Kaaaaaa");
	Animal a3("Mammal", "Mark", "YIPPEEEE!!!");
	a1.makeSound();
	a1.walkForward();
	a2.checkAnimalType(a1);
	a3.declareSelf();
	Animal a4 = a3;	//Using the copy constructor
	a4.declareSelf();
	Animal a5;
	a5.declareSelf();
	a5 = a2;	//Using the operator overload
	a5.declareSelf();
	a2.declareSelf();
	Animal mixed = a1 + a2;
	mixed.declareSelf();
